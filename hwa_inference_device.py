...
from config import LSTM_HWA_INFERENCE_DEVICE_Config
...
def main():
    # setup rpu config
    lstm_config = LSTM_HWA_INFERENCE_DEVICE_Config()
    ...
    total_all_experiments = len(lstm_config.inference_time) * len(lstm_config.noise_scale) * \
                           len(lstm_config.devices)
    print(f"\nTotal experiments to run: {total_all_experiments}")
    
    # inference

    for device in lstm_config.devices:
